...
export const XDSkirtTutorialComponent = ({}:XDSkirtTutorialProps) =>
    <div className={styles.cmsPage}>
        <h1>Handling Skirts and Dresses in XD</h1>
        <div>
            ...
            <p>XD does not have the ability to transfer these custom bones or movement morphs from an original clothing item to a converted clothing item. However, it does have two features which work together to insert some basic posing ability into converted skirts and dresses.</p>
            <h2>Regrouping</h2>
            <p>By default, XD regroups converted clothing so that the clothing bone structure matches the target figure's bone structure. This is necessary since many figures to not have the same hierarchies. For example, Vicky 3 has buttock bones between the hip bone and the thigh bones, while Vicky 4 just has thigh bones. If no regrouping is done, then there can be major posing issues, as seen in the top-left image above.</p>
            <p>However, using the regrouping feature is not enough to support skirts and dresses. XD assigns polygons in the clothing to the nearest bone in the target figure. This means that skirts and dresses would be grouped with the leg bones, which would break the clothing when the figure is posed, as seen in the top-right image above.</p>
            <h2>"Skirt" and "Add skirt handles"</h2>
            <p>To solve this problem, XD has a "Skirt" option which will group all skirt and dress polygons into the hip bone instead of the leg bones. The program will also add five skirt bones to the clothing figure to assist with posing:</p>
            <ul>
            ...
            <p>By checking the "Add skirt handles" box, XD will also add handle geometry to the clothing, so that you don't have to try and find the skirt bones in the body part selector.</p>
